...
print("Image dimensions: {}\n".format(relabel.shape))


## Discovered issue where label() causes a plaque to be split up
## so it showed 8 total instead of 7
# print(label_data.max())
# print(relabel.max())
# print(obj)


plaque_bbox = mh.labeled.bbox(relabel)
# print("Plaque bbox array:\n{}\n".format(plaque_bbox))
# print("Plaque 8 location: {}\n".format(list(zip(np.where(relabel==8)[0], np.where(relabel==8)[1], np.where(relabel==8)[2]))))
...
for i in range(1, len(plaque_bbox)):
	...
	y_height = round((holder[1]-holder[0])*voxel_dimensions[1], 3)
	x_width = round((holder[3]-holder[2])*voxel_dimensions[0], 3)
	z_depth = round((holder[5]-holder[4])*voxel_dimensions[2], 3)
...
# data = "C:/Users/alexa/Desktop/CS 410/data/DICOM 7/Segmentation.seg.nrrd"

# image = nrrd.read(data)
